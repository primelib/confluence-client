package io.github.primelib.confluence4j.rest.v2;

import java.util.Map;
import javax.annotation.processing.Generated;
import lombok.Builder;

import io.github.primelib.confluence4j.rest.v2.api.ConfluenceRESTV2Api;
...
public class ConfluenceRESTV2Factory {
    public static <T> T create(Consumer<ConfluenceRESTV2FactorySpec<T>> spec) {
        ConfluenceRESTV2FactorySpec<T> config = new ConfluenceRESTV2FactorySpec<>(spec);

        if (config.api().isInterface()) {
            return buildClient(spec);
        } else {
            try {
                ConfluenceRESTV2Api api = buildClient(s -> {
                    s.api(ConfluenceRESTV2Api.class);
                    s.applySpec(config);
                });
                return config.api().getConstructor(ConfluenceRESTV2Api.class).newInstance(api);
            } catch (Exception ex) {
                throw new IllegalArgumentException("api must have a constructor with one parameter of type ConfluenceRESTV2Api", ex);
            }
        }
    }

    public static ConfluenceRESTV2Api create() {
        return create(spec -> spec.api(ConfluenceRESTV2Api.class));
    }

    private static <T> T buildClient(Consumer<ConfluenceRESTV2FactorySpec<T>> spec) {
        ConfluenceRESTV2FactorySpec<T> config = new ConfluenceRESTV2FactorySpec<>(spec);
