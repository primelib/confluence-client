...
import io.github.primelib.confluence4j.rest.v1.api.ConfluenceRESTV1Api;
...
public class ConfluenceRESTV1Factory {
    public static <T> T create(Consumer<ConfluenceRESTV1FactorySpec<T>> spec) {
        ConfluenceRESTV1FactorySpec<T> config = new ConfluenceRESTV1FactorySpec<>(spec);

        if (config.api().isInterface()) {
            return buildClient(spec);
        } else {
            try {
                ConfluenceRESTV1Api api = buildClient(s -> {
                    s.api(ConfluenceRESTV1Api.class);
                    s.applySpec(config);
                });
                return config.api().getConstructor(ConfluenceRESTV1Api.class).newInstance(api);
            } catch (Exception ex) {
                throw new IllegalArgumentException("api must have a constructor with one parameter of type ConfluenceRESTV1Api", ex);
            }
        }
    }

    public static ConfluenceRESTV1Api create() {
        return create(spec -> spec.api(ConfluenceRESTV1Api.class));
    }

    private static <T> T buildClient(Consumer<ConfluenceRESTV1FactorySpec<T>> spec) {
        ConfluenceRESTV1FactorySpec<T> config = new ConfluenceRESTV1FactorySpec<>(spec);
