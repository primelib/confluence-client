...
public class GetAndAsyncConvertMacroBodyByMacroIdOperationSpec {
     ...
     * - {@code export_view} - {@code styled_view} - {@code view}
     */
    @NotNull 
    private String to;
    ...
    /**
     * If this field is false, the cache will erase its current value and begin a conversion. If this field is true, the cache will not erase its current value, and will set the status of the result in cache to RERUNNING. Once the data is updated, the status will change to COMPLETED.
     * Large macros that take long to convert, and who want to show intermediate, but potentially stale data, immediately should set this field to true. Cache values are stored per macro per user per content and expansions.
     */
    @Nullable 
    ...
    /**
     * Constructs a validated instance of {@link GetAndAsyncConvertMacroBodyByMacroIdOperationSpec}.
     *
     * @param spec the specification to process
     */
    @ApiStatus.Internal
    public GetAndAsyncConvertMacroBodyByMacroIdOperationSpec(Consumer<GetAndAsyncConvertMacroBodyByMacroIdOperationSpec> spec) {
        spec.accept(this);
     ...
     * @param macroId              The ID of the macro. For apps, this is passed to the macro by the Connect/Forge framework. Otherwise, find the macro ID by querying the desired content and version, then expanding the body in storage format. For example, '/content/196611/version/7?expand=content.body.storage'.
     * @param to                   The content representation to return the macro in. Currently, the following conversions are allowed:  - {@code export_view} - {@code styled_view} - {@code view}
     ...
     */
    @ApiStatus.Internal
    public GetAndAsyncConvertMacroBodyByMacroIdOperationSpec(String id, Integer version, String macroId, String to, List<String> expand, Boolean allowCache, String spaceKeyContext, String embeddedContentRender) {
        this.id = id;
