package io.github.primelib.confluence4j.rest.v1.model;

import java.util.function.Consumer;
import org.jetbrains.annotations.ApiStatus;
import javax.annotation.processing.Generated;
import lombok.AccessLevel;
import lombok.Builder;
import lombok.Getter;
import lombok.Setter;
import lombok.EqualsAndHashCode;
import lombok.ToString;
import lombok.NoArgsConstructor;
import lombok.AllArgsConstructor;
import lombok.experimental.Accessors;

import com.fasterxml.jackson.annotation.JsonPropertyOrder;
import com.fasterxml.jackson.annotation.JsonTypeName;
import com.fasterxml.jackson.annotation.JsonProperty;
import com.fasterxml.jackson.annotation.JsonCreator;
import com.fasterxml.jackson.annotation.JsonValue;

/**
 * LookAndFeelSettings
 *
 */
@Getter
@Setter
@EqualsAndHashCode
@ToString
@Accessors(fluent = true, chain = true)
@NoArgsConstructor(access = AccessLevel.PROTECTED, force = true)
@Builder
@JsonPropertyOrder({
    "selected",
    "global",
    "theme",
    "custom"
})
@JsonTypeName("LookAndFeelSettings")
@Generated(value = "io.github.primelib.primecodegen.javafeign.JavaFeignGenerator")
public class LookAndFeelSettings {

    @JsonProperty("selected")
    protected SelectedEnum selected;

    @JsonProperty("global")
    protected LookAndFeel global;

    @JsonProperty("theme")
    protected LookAndFeel theme;

    @JsonProperty("custom")
    protected LookAndFeel custom;

    /**
     * Constructs a validated instance of {@link LookAndFeelSettings}.
     *
     * @param spec the specification to process
     */
    public LookAndFeelSettings(Consumer<LookAndFeelSettings> spec) {
        spec.accept(this);
    }

    /**
     * Constructs a validated instance of {@link LookAndFeelSettings}.
     * <p>
     * NOTE: This constructor is not considered stable and may change if the model is updated. Consider using {@link #LookAndFeelSettings(Consumer)} instead.
     * @param selected selected
     * @param global global
     * @param theme theme
     * @param custom custom
     */
    @ApiStatus.Internal
    public LookAndFeelSettings(SelectedEnum selected, LookAndFeel global, LookAndFeel theme, LookAndFeel custom) {
        this.selected = selected;
        this.global = global;
        this.theme = theme;
        this.custom = custom;
    }

    @AllArgsConstructor
    public enum SelectedEnum {
        GLOBAL("global"),
        CUSTOM("custom");

        private static final SelectedEnum[] VALUES = values(); // prevent allocating a new array for every call to values()
        private final String value;

        @JsonCreator
        public static SelectedEnum of(String input) {
            if (input != null) {
                for (SelectedEnum v : VALUES) {
                    if (input.equalsIgnoreCase(v.value)) 
                        return v;
                }
            }

            return null;
        }

        @JsonValue
        public String getValue() {
            return value;
        }
    }

}
