package io.github.primelib.confluence4j.rest.v1.spec;

import java.util.Objects;
import org.jetbrains.annotations.ApiStatus;
import org.jetbrains.annotations.NotNull;
import javax.annotation.processing.Generated;

import io.github.primelib.confluence4j.rest.v1.model.Task;
import io.github.primelib.confluence4j.rest.v1.model.TaskStatusUpdate;
import lombok.AccessLevel;
import lombok.Builder;
import lombok.Getter;
import lombok.Setter;
import lombok.EqualsAndHashCode;
import lombok.ToString;
import lombok.NoArgsConstructor;
import lombok.experimental.Accessors;
import java.util.function.Consumer;

/**
 * UpdateTaskByIdSpec
 * <p>
 * Specification for the UpdateTaskById operation.
 * <p>
 * Update inline task given global ID
 */
@Getter
@Setter
@EqualsAndHashCode
@ToString
@Accessors(fluent = true, chain = true)
@NoArgsConstructor(access = AccessLevel.PROTECTED, force = true)
@Builder
@Generated(value = "io.github.primelib.primecodegen.javafeign.JavaFeignGenerator")
public class UpdateTaskByIdOperationSpec {
    /**
     * allows to disable validation of the spec, use with care!
     */
    @ApiStatus.Experimental
    public static Boolean VALIDATION_ENABLED = true;

    /**
     * Global ID of the inline task to update
     */
    @NotNull 
    private String inlineTaskId;

    /**
     * The updated task status.
     */
    @NotNull 
    private TaskStatusUpdate taskStatusUpdate;

    /**
     * Constructs a validated instance of {@link UpdateTaskByIdOperationSpec}.
     *
     * @param spec the specification to process
     */
    @ApiStatus.Internal
    public UpdateTaskByIdOperationSpec(Consumer<UpdateTaskByIdOperationSpec> spec) {
        spec.accept(this);
        if (VALIDATION_ENABLED)
            validate();
    }

    /**
     * Constructs a validated instance of {@link UpdateTaskByIdOperationSpec}.
     * <p>
     * NOTE: This constructor is not considered stable and may change if the operation is updated.
     * @param inlineTaskId         Global ID of the inline task to update
     * @param taskStatusUpdate     The updated task status.
     */
    @ApiStatus.Internal
    public UpdateTaskByIdOperationSpec(String inlineTaskId, TaskStatusUpdate taskStatusUpdate) {
        this.inlineTaskId = inlineTaskId;
        this.taskStatusUpdate = taskStatusUpdate;

        if (VALIDATION_ENABLED)
            validate();
    }

    /**
     * Validates the Spec, will throw a exception if required parameters are missing
     *
     * @throws NullPointerException
     */
    public void validate() {
        Objects.requireNonNull(inlineTaskId, "inlineTaskId is a required parameter!");
        Objects.requireNonNull(taskStatusUpdate, "taskStatusUpdate is a required parameter!");
    }
}
