package io.github.primelib.confluence4j.rest.v1.spec;

import java.util.Objects;
import org.jetbrains.annotations.ApiStatus;
import org.jetbrains.annotations.NotNull;
import javax.annotation.processing.Generated;

import io.github.primelib.confluence4j.rest.v1.model.Space;
import io.github.primelib.confluence4j.rest.v1.model.SpaceSettings;
import io.github.primelib.confluence4j.rest.v1.model.SpaceSettingsUpdate;
import lombok.AccessLevel;
import lombok.Builder;
import lombok.Getter;
import lombok.Setter;
import lombok.EqualsAndHashCode;
import lombok.ToString;
import lombok.NoArgsConstructor;
import lombok.experimental.Accessors;
import java.util.function.Consumer;

/**
 * UpdateSpaceSettingsSpec
 * <p>
 * Specification for the UpdateSpaceSettings operation.
 * <p>
 * Update space settings
 */
@Getter
@Setter
@EqualsAndHashCode
@ToString
@Accessors(fluent = true, chain = true)
@NoArgsConstructor(access = AccessLevel.PROTECTED, force = true)
@Builder
@Generated(value = "io.github.primelib.primecodegen.javafeign.JavaFeignGenerator")
public class UpdateSpaceSettingsOperationSpec {
    /**
     * allows to disable validation of the spec, use with care!
     */
    @ApiStatus.Experimental
    public static Boolean VALIDATION_ENABLED = true;

    /**
     * The key of the space whose settings will be updated.
     */
    @NotNull 
    private String spaceKey;

    /**
     * The space settings to update.
     */
    @NotNull 
    private SpaceSettingsUpdate spaceSettingsUpdate;

    /**
     * Constructs a validated instance of {@link UpdateSpaceSettingsOperationSpec}.
     *
     * @param spec the specification to process
     */
    @ApiStatus.Internal
    public UpdateSpaceSettingsOperationSpec(Consumer<UpdateSpaceSettingsOperationSpec> spec) {
        spec.accept(this);
        if (VALIDATION_ENABLED)
            validate();
    }

    /**
     * Constructs a validated instance of {@link UpdateSpaceSettingsOperationSpec}.
     * <p>
     * NOTE: This constructor is not considered stable and may change if the operation is updated.
     * @param spaceKey             The key of the space whose settings will be updated.
     * @param spaceSettingsUpdate  The space settings to update.
     */
    @ApiStatus.Internal
    public UpdateSpaceSettingsOperationSpec(String spaceKey, SpaceSettingsUpdate spaceSettingsUpdate) {
        this.spaceKey = spaceKey;
        this.spaceSettingsUpdate = spaceSettingsUpdate;

        if (VALIDATION_ENABLED)
            validate();
    }

    /**
     * Validates the Spec, will throw a exception if required parameters are missing
     *
     * @throws NullPointerException
     */
    public void validate() {
        Objects.requireNonNull(spaceKey, "spaceKey is a required parameter!");
        Objects.requireNonNull(spaceSettingsUpdate, "spaceSettingsUpdate is a required parameter!");
    }
}
